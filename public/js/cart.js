...
async function checkLoginStatus() {
    try {
        ...
        // Validate token
        const validateResponse = await fetch(`${window.config.API_URL}/api/auth/validate`, {
            headers: {
        ...
        const cartResponse = await fetch(`${window.config.API_URL}/api/cart`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function syncCartWithDB() {
    const token = localStorage.getItem('token');
    if (!window.isLoggedIn || !token) {
        console.log('User not logged in, skipping cart sync');
        return;
    }

    try {
        const response = await fetch(`${window.config.API_URL}/api/cart/sync`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(error.message || 'Failed to sync cart with database');
        }
    } catch (error) {
        console.error('Error syncing cart:', error);
        showNotification(error.message || 'Failed to sync cart with server', 'error');
...
async function handleLogin(e) {
    try {
        ...
        if (data.token) {
            ...
            // Hide login modal and show success message
            document.getElementById('loginModal').style.display = 'none';
            showNotification('Login successful!');
            
            // Fetch and merge cart after successful login
            const cartResponse = await fetch(`${window.config.API_URL}/api/cart`, {
                headers: {
...
async function handleRazorpayPayment(response, order) {
    try {
        ...
        const verifyResponse = await fetch(`${window.config.API_URL}/api/order/verify-payment`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
async function orderSuccess(order) {
    try {
        ...
        window.location.href = `/profile.html?orderId=${order._id}`;
    } catch (error) {
